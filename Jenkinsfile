        ...
        stage('Run Container on Dev Server') {
          steps {
            ...
            sshagent(['ec2-ssh']) {
                ...
                sh "ssh -o StrictHostKeyChecking=no ${username}@${ip} 'docker ps -f -fname=${springname} -q | xargs -r docker container rm'"
                sh "ssh -o StrictHostKeyChecking=no ${username}@${ip} 'docker images -f reference=${imagename}:${tagname} -q | xargs -r docker image rmi'"
